...
from metric import Voice, MusicNote, TimePointSequence
...
class VoiceSplittingModelState(model_base.MidiModelState, ABC):
    ...
    def get_current_voices(self) -> List[Voice]:
        raise NotImplementedError("Method get_current_voices() not implemented!!!!")

    def remove_note(self, note: MusicNote, min_note_length: int = 100000):
    ...
    def keep_note(self, note: MusicNote, min_note_length: int = 100000):
...
class VoiceSplittingGrammarModelState(VoiceSplittingModelState):
    def __init__(self, sequence: TimePointSequence) -> None:
        ...
        while current_tp is not None:
            notes_at_timepoint: DefaultDict[int, List[MusicNote]] = sequence.get_notes_for_timepoint(current_tp)
            ...
            for idx, notes_of_voice in notes_at_timepoint.items():
                ...
                for n in notes_of_voice:
                    old_voices: List[Voice] = self.voices[idx]
                    if len(old_voices) == 0:
                        new_voices.append(Voice(current_note=n))
                    else:
                        for ov in old_voices:
                            new_voices.append(Voice(current_note=n, prev_voice=ov))
    ...
    def transition(self, notes: List[MusicNote] = None) -> OrderedDictType[model_base.MidiModelState]:
